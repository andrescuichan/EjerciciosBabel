...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable()
		.authorizeRequests()
		.antMatchers(HttpMethod.GET,"/Reservass").hasRole("ADMIN") //esa url no existe pero es para que funcione sin auntentificar con el cliente
		//.antMatchers(HttpMethod.PUT,"/Alumno").hasRole("ADMIN")
		//.antMatchers(HttpMethod.DELETE,"/Alumno/*").hasRole("OPERATOR")
		//.antMatchers(HttpMethod.GET, "/Alumno/*").authenticated()	// Cualquiera quien este autentificado
		.and()
